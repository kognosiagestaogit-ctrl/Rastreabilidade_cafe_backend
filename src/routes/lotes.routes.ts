...
const loteSchema = z.object({
  ...
  data_entrada_terreiro_inicio: z.string().optional().nullable(),
  data_entrada_terreiro_fim: z.string().optional().nullable(),
...
lotesRouter.post("/lotes", async (c) => {
  try {
    ...
    const novo = {
      ...
      data_entrada_terreiro_inicio: data.data_entrada_terreiro_inicio ?? null,
      data_entrada_terreiro_fim: data.data_entrada_terreiro_fim ?? null,
    ...
    if (created.numero_lote_cooperativa) {
      ...
      if (vendaEncontrada) {
        ...
        await db
          .update(vendasTable)
          .set({ lote_id: created.id, sobras_sacas: sobra })
  ...
  try {
    ...
    if (updated.numero_lote_cooperativa) {
      ...
      if (vendaEncontrada) {
        ...
        await db
          .update(vendasTable)
          .set({ lote_id: updated.id, sobras_sacas: sobra, sacas_do_lote: updated.numero_sacas })
          .where(eq(vendasTable.id, vendaEncontrada.id));
